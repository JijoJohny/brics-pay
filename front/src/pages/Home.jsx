  ...
  return (
    <div className='h-screen flex overflow-hidden'>
      <div className='fixed bg-gradient-to-br from-slate-900 to-slate-800 w-full sm:w-[20%] h-full flex flex-col gap-3 p-3'>
        <div className='text-center text-white text-4xl mb-5 mt-5 font-bold'><span className='text-[#2e72d2]'>ß</span>RICSPAY</div>
        <div className='flex flex-col gap-3 text-white'>
          <div className={`${sideclass} ${selected === 'Balance' ? 'border-b-4 border-[#2e72d2]' : ''}`} onClick={() => scrollToRef(balanceRef, 'Balance')}>
            <BalanceIcon />
            <div>Balance</div>
          </div>
          <div className={`${sideclass} ${selected === 'BuyTokens' ? 'border-b-4 border-[#2e72d2]' : ''}`} onClick={() => scrollToRef(buyTokensRef, 'BuyTokens')}>
            <CurrencyExchangeIcon />
            <div>Buy Tokens</div>
          </div>
          <div className={`${sideclass} ${selected === 'Transaction' ? 'border-b-4 border-[#2e72d2]' : ''}`} onClick={() => scrollToRef(transactionRef, 'Transaction')}>
            <MultipleStopIcon />
            <div>Transaction</div>
          </div>
          <div className={`${sideclass} ${selected === 'History' ? 'border-b-4 border-[#2e72d2]' : ''}`} onClick={() => scrollToRef(historyRef, 'History')}>
            <HistoryIcon />
            <div>History</div>
          </div>
          <div className={`${sideclass} ${selected === 'Stock' ? 'border-b-4 border-[#2e72d2]' : ''}`} onClick={() => scrollToRef(stockRef, 'Stock')}>
            <TrendingUpIcon />
            <div>Stock</div>
          </div>
          <div className={`${sideclass} ${selected === 'Swap' ? 'border-b-4 border-[#2e72d2]' : ''}`} onClick={() => scrollToRef(swapRef, 'Swap')}>
            <SwapVertIcon />
            <div>Swap</div>
          </div>
        </div>
